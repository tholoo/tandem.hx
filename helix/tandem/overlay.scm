...
(define (draw-picker area frame)
  (let* ([available (max 1 (- (ui.area-width area) (chat.chat-width)))]
    ...
    (for-each (lambda (row)
               ...
               (write frame (+ x width -1) (+ y row) 1 "│" border))
      (range 0 height))
      ...
      (for-each (lambda (row)
                 (let* ([index (+ row (unbox offset))]
                        ...
                        [short (if (> (string-length label) limit)
                                (string-append (substring label 0 (- limit 1)) "…")
                                label)]
    ...
    (write frame (+ x 2) (+ y height -3) (- width 4)
      (string-append (number->string (length items)) " / " (number->string (length (unbox options))) " stops")
      muted)
...
(define (handle state event)
  (when (ui.key-event? event)
    (cond [(ui.key-event-escape? event) (close-overlay)]
      [(ui.key-event-up? event) (move -1)]
      [(ui.key-event-down? event) (move 1)]
      [(ui.key-event-page-up? event) (move -10)]
      [(ui.key-event-page-down? event) (move 10)]

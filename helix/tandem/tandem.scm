...
(define (install-keybindings bindings)
  (keys.keymap-update-documentation! bindings
    (hash "tandem" "Open Tandem conversation"
          "tandem-stop" "Stop Tandem session"
          "tandem-hide" "Hide conversation"
          "tandem-next" "Next tour stop or change"
          "tandem-previous" "Previous tour stop or change"
          "tandem-apply" "Apply proposal"
          "tandem-revert" "Revert current change"
          "tandem-close" "Close tour"
          "tandem-peek" "Compare saved change"
          "tandem-return" "Return to tour stop"
          "tandem-stops" "Search tour stops"
          "tandem-compose" "Edit message"
          "tandem-send" "Send message"
          "tandem-compose-close" "Keep draft and return to chat"))
...
(define (stop-location stop)
  (let* ([start (inexact->exact (hash-ref stop 'line))]
    ...
    (string-append (hash-ref stop 'file) ":" (number->string start)
      ...
      " · " (number->string count) (if (= count 1) " line" " lines"))))
...
(define (publish-context)
  ...
  (when (and (unbox output) (not (unbox navigating)) (has-editor-view?))
    (let* ([doc (editor->doc-id (editor-focus))]
           ...
           [from (editor.range->from selection)] [to (editor.range->to selection)]
      ...
      (unless (or (compose.composer-doc? doc) (compare.comparison-doc? doc))
        (let ([context (hash 'file (if path path void)
                        ...
                        'selection_start_line (if selected (+ 1 (text.rope-char->line rope from)) void)
                        'selection_end_line (if selected (+ 1 (text.rope-char->line rope (max from (- to 1)))) void)
                        'nearby (text.rope->string (text.rope->slice rope start end)) 'dirty dirty)])
    ...
    (when (hash? tour)
      (let* ([index (inexact->exact (hash-ref tour 'current_stop))]
        ...
        (unless (equal? identity (unbox card-stop))
          (set-box! card-stop identity) (set-box! card-scroll 0))
    ...
    (when (hash? next)
      (let* ([before (active-tour)] [after (field next 'tour #f)]
        ...
        (when (and (repository? before) (not (hash? after)) (equal? (field next 'stage "") "discuss"))
          (set-box! navigating #t) (loc.restore (unbox tour-origin)) (set-box! navigating #f)
      ...
      (let ([tour (active-tour)])
        (cond [(and (hash? tour) (not (unbox card-visible)))
              ...
              [(and (not (hash? tour)) (unbox card-visible))
               ...
               (set-editor-clip-top! 0) (pop-last-component-by-name! "tandem-tour")]))
...
(let ([defaults (keys.helix-string->keymap (value->jsexpr-string
        (hash "normal" (hash "space" (hash "t"
          (hash "p" ":tandem-peek" "r" ":tandem-return" "j" ":tandem-stops"
                "e" ":tandem-compose"
                "s" ":tandem-send" "q" ":tandem-compose-close"))))))])

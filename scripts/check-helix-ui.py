...
plugin = (
    Path(sys.argv[3])
    if len(sys.argv) > 3
    else Path(__file__).resolve().parents[1] / "helix/tandem"
)
...
source.write_text(
    source.read_text()
    .replace("source line 120", "source line 120 changed")
...
view = dict(
    ...
    tour=dict(
        ...
        stops=[
            dict(
                title="The request handler",
                body="Credentials are parsed here. Run `:tandem` to ask a question.",
                file="story.txt",
                line=120,
                end_line=135,
            )
...
(helper / "tandem").write_text(
    ...
    + " with open("
    + repr(str(root / "requests.jsonl"))
    + ", 'a') as log: log.write(line)\n"
...
try:
    ...
    while time.monotonic() < deadline:
        drain()
        if locate("Assistant:") and locate("The request handler"):
            break
        if process.poll() is not None or b"error[E" in capture:
            break
        time.sleep(0.05)
    time.sleep(0.3)
    drain()
    if b"error[E" in capture or b"panicked" in capture:
        raise RuntimeError(
            f"Steel initialization check failed; inspect {root / 'terminal.bin'}"
        )
    for label in ["You:", "Assistant:", "/next", ":tandem"]:
        pos = locate(label)
        if pos is None:
            errors.append(f"missing rendered {label}")
    if all(locate(t) for t in ["You:", "Assistant:", "/next", ":tandem"]):
        colors = {
            t: screen.buffer[locate(t)[0]][locate(t)[1]].fg
            for t in ["You:", "Assistant:", "/next", ":tandem"]
        }
        (root / "colors.json").write_text(json.dumps(colors))
        if colors["You:"] == colors["Assistant:"]:
            errors.append(f"role labels share a color: {colors}")
        if colors["/next"] == "e6e1cf" or colors[":tandem"] == "e6e1cf":
            errors.append(f"commands have plain-text color: {colors}")
    ...
    if target is None or not 5 <= target[0] <= 10:
        errors.append(f"reading range is not framed from its start: {target}")
    ...
    if target:
        ...
        if not any(mark in before for mark in ["▌", "▎", "▍", "▏", "▐", "┆"]):
            errors.append(
                f"Git change indicator missing beside reading range: {before!r}"
            )
    narration = locate("The request handler")
    if narration is None or narration[0] > 2:
        errors.append(f"narration is not above the code: {narration}")
    ...
    wait_screen(
        lambda: (
            locate("source line 130") is not None and locate("source line 130")[0] < 10
        ),
        "scroll did not move range",
    )
    ...
    wait_screen(
        lambda: (
            screen.display[-1].rstrip().endswith("> Unsent draft")
    ...
    wait_screen(
        lambda: screen.display[-1].rstrip().endswith("> /next"),
        "Up did not recall the newest message",
    )
    os.write(master, b"\x1b[A")
    wait_screen(
        lambda: screen.display[-1].rstrip().endswith("> Two lines↵Second line"),
        "Up did not recall multiline message past a slash command",
    )
    os.write(master, b"\x1b[A edited")
    wait_screen(
        lambda: screen.display[-1].rstrip().endswith("> First question edited"),
        "recalled message could not be edited",
    )
    os.write(master, b"\x1b[B\x1b[A")
    wait_screen(
        lambda: screen.display[-1].rstrip().endswith("> First question edited"),
        "history browsing lost edits",
    )
    os.write(master, b"\x1b[B\x1b[B\x1b[B")
    wait_screen(
        lambda: (
            screen.display[-1].rstrip().endswith("> Unsent draft")
            and screen.cursor.x == draft_cursor
        ),
    ...
    def resubmitted():
        return any(
            json.loads(line).get("text") == "Two lines\nSecond line!"
    ...
    wait_screen(
        lambda: screen.display[-1].rstrip().endswith("> /apply"),
        "Down in slash menu recalled history instead of selecting a command",
    )
    ...
    os.write(master, b" t")
    wait_screen(
        lambda: locate("Return to tour stop") is not None,
        "Tandem key menu has no descriptions",
    )
    ...
    for description in [
        ...
        if not locate(description):
            errors.append(f"missing key description: {description}")
    os.write(master, b"\x1b")
    wait_screen(
        lambda: (
            locate("Return to tour stop") is None
            and locate("Undocumented plugin command") is None
        ),
    ...
    wait_screen(
        lambda: locate("No matching stops") is not None,
        "picker did not show empty search",
    )
    os.write(master, b"\x15rqhd")
    wait_screen(
        lambda: locate("1 / 2 stops") is not None, "picker did not fuzzy-match the stop"
    )
    (root / "tour-picker.txt").write_text("\n".join(screen.display))
    borders = [
        (row, line.find("╭")) for row, line in enumerate(screen.display) if "╭" in line
    ]
    if not borders:
        errors.append("picker has no border")
    if not locate("Enter jump"):
        errors.append("picker keyboard hint missing")
    os.write(master, b"\x1b")
    ...
    wait_screen(
        lambda: locate("Comparison unavailable") is not None,
        "successful response without comparison was silently ignored",
    )
    os.write(master, b" tp")
    wait_screen(
        lambda: locate("Old · story.txt") is not None, "native comparison did not open"
    )
    ...
    wait_screen(
        lambda: locate("EDITED source") is not None,
        "current comparison side is not editable",
    )
    ...
    os.write(master, b" tp")
    wait_screen(
        lambda: locate("Tandem ·") is not None,
        "closing the baseline window broke comparison cleanup",
    )
    wait_screen(
        lambda: locate("EDITED source") is not None,
        "closing comparison lost current edits",
    )
    dirty = [
        json.loads(line).get("context", {}).get("dirty", [])
        for line in (root / "requests.jsonl").read_text().splitlines()
    ]
    ...
    while process.poll() is None and time.monotonic() < deadline:
        ...
        time.sleep(0.05)
    drain()
    if process.poll() != 0 or b"panicked" in capture:
        errors.append(f"q! did not exit cleanly: {process.poll()}")
finally:
...
if errors:
    sys.exit(1)
print(
    "PASS: narration range, native range selection, Git coexistence, wrapping, movement, styled chat, message recall/resend, shortcut descriptions, editable native comparison/cleanup, and q! shutdown"
)
